  ...
  const onFileDrop =async (e) => {
    ...
    if (newFile && isValidFileType(newFile)) {
      ...
      if (fileList.length === 0) {
        const pages = await countPages(newFile);
        filePage = pages;
        const updatedList = [...fileList, newFile];
        fileList2 = [...fileList, newFile];
    ...
    if (file.type === 'application/pdf') {
      ...
      return new Promise((resolve, reject) => {
        fileReader.onload = function () {
          ...
          const matchResults = content.match(/\/Page\W/g);
          const numPages = matchResults ? matchResults.length : 0;
          resolve(numPages);
        };
...
export { DropFileInput, fileList2,filePage };
